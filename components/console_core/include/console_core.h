 ...
 *
 * This component provides a unified command-line interface for the robOS
 * system, including UART interface, command parser, help system, and command
 * registration.
 *
 * Features:
...
#define CONSOLE_MAX_COMMANDS                                                   \
  (64) ///< Maximum number of registered commands (increased for color
       ///< correction)
 ...
 */
typedef enum {
  TEMP_SOURCE_DEFAULT = 0, ///< Use default temperature (25°C)
  TEMP_SOURCE_MANUAL,      ///< Use manually set test temperature
  TEMP_SOURCE_AGX_AUTO     ///< Use AGX CPU temperature automatically
} temp_source_type_t;

/**
 * @brief Set test temperature value for debugging (manual mode)
 *
...
/**
 * @brief Get current effective temperature value
 *
...
/**
 * @brief Get current temperature with priority system
 *
 * Priority: Manual > AGX Auto > Default
 *
 * @param temperature Pointer to store temperature value
 * @param source Pointer to store temperature source type (optional)
 * @return esp_err_t ESP_OK on success, error code on failure
 */
...
/**
 * @brief Set AGX CPU temperature for automatic fan control
 *
 * @param temperature AGX CPU temperature in Celsius
 * @return esp_err_t ESP_OK on success, error code on failure
 */
esp_err_t console_set_agx_temperature(float temperature);

/**
 * @brief Enable/disable manual temperature mode
 *
 * @param enable true to enable manual mode, false to use AGX auto mode
 * @return esp_err_t ESP_OK on success, error code on failure
 */
esp_err_t console_set_manual_temp_mode(bool enable);

/**
 * @brief Check if manual temperature mode is enabled
 *
 * @param enabled Pointer to store manual mode status
 * @return esp_err_t ESP_OK on success, error code on failure
 */
