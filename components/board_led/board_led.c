...
bool board_led_is_initialized(void) { return s_board_led.is_initialized; }

/**
 * @brief Internal function to set pixel without stopping animation
 */
static esp_err_t board_led_set_pixel_internal(uint16_t index,
  ...
  if (index >= BOARD_LED_COUNT) {
    ESP_LOGE(TAG, "Invalid LED index: %d (max: %d)", index,
             BOARD_LED_COUNT - 1);
  ...
  esp_err_t ret = led_strip_set_pixel(s_board_led.led_strip, index, color.red,
...
esp_err_t board_led_set_pixel(uint16_t index, board_led_color_t color) {
  ...
  if (xSemaphoreTake(s_board_led.mutex,
                     pdMS_TO_TICKS(BOARD_LED_MUTEX_TIMEOUT_MS)) != pdTRUE) {
...
esp_err_t board_led_set_all_color(board_led_color_t color) {
  ...
  for (uint16_t i = 0; i < BOARD_LED_COUNT; i++) {
    esp_err_t err = led_strip_set_pixel(s_board_led.led_strip, i, color.red,
                                        color.green, color.blue);
    if (err != ESP_OK) {
      ret = err;
    }
...
esp_err_t board_led_set_brightness(uint8_t brightness) {
  ...
  if (s_board_led.has_static_color) {
    ...
    for (uint16_t i = 0; i < BOARD_LED_COUNT; i++) {
      led_strip_set_pixel(s_board_led.led_strip, i, color.red, color.green,
                          color.blue);
    }
    led_strip_refresh(s_board_led.led_strip);
  }
...
esp_err_t board_led_start_animation(board_led_animation_t animation,
                                    uint8_t speed,
                                    board_led_color_t primary_color,
                                    board_led_color_t secondary_color) {
  if (!s_board_led.is_initialized) {
    return ESP_ERR_INVALID_STATE;
  }
  ...
  if (animation != BOARD_LED_ANIM_NONE) {
    ...
    BaseType_t task_created = xTaskCreate(
        animation_task, "board_led_anim", BOARD_LED_ANIMATION_TASK_STACK_SIZE,
        NULL, BOARD_LED_ANIMATION_TASK_PRIORITY,
...
static void animate_fade(void) {
  uint8_t fade_value =
      (uint8_t)((sin(s_board_led.animation_step * 0.1) + 1) * 127);
...
static void animate_rainbow(void) {
  for (uint16_t i = 0; i < BOARD_LED_COUNT; i++) {
    uint16_t hue =
        (s_board_led.animation_step * 10 + i * 360 / BOARD_LED_COUNT) % 360;
...
static void animate_breathe(void) {
  uint8_t breath_value =
      (uint8_t)((sin(s_board_led.animation_step * 0.2) + 1) * 127);
  board_led_color_t color = s_board_led.animation_primary_color;
...
static void animate_wave(void) {
  for (uint16_t i = 0; i < BOARD_LED_COUNT; i++) {
    uint8_t wave_value =
        (uint8_t)((sin((s_board_led.animation_step + i * 5) * 0.3) + 1) * 127);
    board_led_color_t color =
        board_led_blend_colors(s_board_led.animation_secondary_color,
                               s_board_led.animation_primary_color, wave_value);
    board_led_set_pixel_internal(i, color);
  }
}
...
static void animate_chase(void) {
  ...
  for (uint16_t i = 0; i < BOARD_LED_COUNT; i++) {
    ...
    } else if ((i + 1) % BOARD_LED_COUNT == chase_pos ||
               (i + BOARD_LED_COUNT - 1) % BOARD_LED_COUNT == chase_pos) {
...
static void animate_fire(void) {
  for (uint16_t i = 0; i < BOARD_LED_COUNT; i++) {
    ...
    board_led_color_t color = {.red = flicker, .green = flicker / 3, .blue = 0};
    board_led_set_pixel_internal(i, color);
  }
}
...
board_led_color_t board_led_hsv_to_rgb(uint16_t hue, uint8_t saturation,
                                       uint8_t value) {
...
board_led_color_t board_led_blend_colors(board_led_color_t color1,
                                         board_led_color_t color2,
                                         uint8_t ratio) {
  board_led_color_t result;

  result.red = ((color1.red * (255 - ratio)) + (color2.red * ratio)) / 255;
  result.green =
      ((color1.green * (255 - ratio)) + (color2.green * ratio)) / 255;
...
static void animate_gradient(void) {
  for (uint16_t i = 0; i < BOARD_LED_COUNT; i++) {
    ...
    board_led_color_t color =
        board_led_blend_colors(s_board_led.animation_primary_color,
                               s_board_led.animation_secondary_color, ratio);
    board_led_set_pixel_internal(i, color);
  }
}
...
static void animate_rainbow_wave(void) {
  for (uint16_t i = 0; i < BOARD_LED_COUNT; i++) {
    uint16_t hue =
        (s_board_led.animation_step * 5 + i * 360 / BOARD_LED_COUNT) % 360;
    board_led_color_t color = board_led_hsv_to_rgb(hue, 100, 100);
    board_led_set_pixel_internal(i, color);
  }
}
...
static void animate_sparkle(void) {
  ...
  for (uint16_t i = 0; i < BOARD_LED_COUNT; i++) {
    if (sparkle_brightness[i] > 0) {
      sparkle_brightness[i] =
          (sparkle_brightness[i] > 10) ? sparkle_brightness[i] - 10 : 0;
    }
  }
...
esp_err_t board_led_save_config(void) {
  ...
  board_led_saved_config_t saved_config = {
      ...
      .static_color = s_board_led.has_static_color
                          ? s_board_led.current_static_color
                          : (board_led_color_t){0, 0, 0},
  ...
  esp_err_t ret =
      config_manager_set(BOARD_LED_CONFIG_NAMESPACE, BOARD_LED_CONFIG_KEY,
                         CONFIG_TYPE_BLOB, &saved_config, sizeof(saved_config));

  if (ret == ESP_OK) {
    ESP_LOGI(TAG,
             "Board LED configuration saved (brightness: %d, animation: %d, "
             "running: %s)",
...
esp_err_t board_led_load_config(void) {
  ...
  esp_err_t ret =
      config_manager_get(BOARD_LED_CONFIG_NAMESPACE, BOARD_LED_CONFIG_KEY,
                         CONFIG_TYPE_BLOB, &saved_config, &config_size);

  if (ret != ESP_OK) {
    ESP_LOGW(TAG, "No saved board LED configuration found: %s",
             esp_err_to_name(ret));
    return ret;
  }
  ...
  if (saved_config.version != BOARD_LED_CONFIG_VERSION) {
    ESP_LOGW(
        TAG,
        "Board LED configuration version mismatch (saved: %d, expected: %d)",
  ...
  if (saved_config.animation_running &&
      saved_config.animation != BOARD_LED_ANIM_NONE) {
    board_led_start_animation(
  ...
  ESP_LOGI(TAG,
           "Board LED configuration loaded (brightness: %d, animation: %d, "
           "static: %s)",
           saved_config.brightness, saved_config.animation,
...
esp_err_t board_led_reset_config(void) {
  ...
  esp_err_t ret =
      config_manager_delete(BOARD_LED_CONFIG_NAMESPACE, BOARD_LED_CONFIG_KEY);
  if (ret == ESP_OK) {
  ...
  if (strcasecmp(argv[1], "board") == 0) {
    if (argc < 3) {
      ...
      if (strcasecmp(argv[3], "stop") == 0) {
        ...
        int b = atoi(argv[6]);
        board_led_color_t color = {(uint8_t)r, (uint8_t)g, (uint8_t)b};
        board_led_start_animation(BOARD_LED_ANIM_BREATHE, speed, color,
                                  BOARD_LED_COLOR_BLACK);
        ...
        int b = atoi(argv[6]);
        board_led_color_t color = {(uint8_t)r, (uint8_t)g, (uint8_t)b};
        board_led_start_animation(BOARD_LED_ANIM_CHASE, speed, color,
                                  BOARD_LED_COLOR_BLACK);
        ...
        int b = atoi(argv[6]);
        board_led_color_t color = {(uint8_t)r, (uint8_t)g, (uint8_t)b};
        board_led_start_animation(BOARD_LED_ANIM_FADE, speed, color,
                                  BOARD_LED_COLOR_BLACK);
        ...
        int b = atoi(argv[6]);
        board_led_color_t color = {(uint8_t)r, (uint8_t)g, (uint8_t)b};
        board_led_start_animation(BOARD_LED_ANIM_WAVE, speed, color,
                                  BOARD_LED_COLOR_BLACK);
        ...
        int b = atoi(argv[6]);
        board_led_color_t color = {(uint8_t)r, (uint8_t)g, (uint8_t)b};
        board_led_start_animation(BOARD_LED_ANIM_TWINKLE, speed, color,
                                  BOARD_LED_COLOR_BLACK);
        ...
        int b = atoi(argv[6]);
        board_led_color_t color = {(uint8_t)r, (uint8_t)g, (uint8_t)b};
        board_led_start_animation(BOARD_LED_ANIM_PULSE, speed, color,
                                  BOARD_LED_COLOR_BLACK);
        ...
        board_led_start_animation(BOARD_LED_ANIM_GRADIENT, speed, color1,
                                  color2);
        ...
        int b = atoi(argv[6]);
        board_led_color_t color = {(uint8_t)r, (uint8_t)g, (uint8_t)b};
        board_led_start_animation(BOARD_LED_ANIM_BRIGHTNESS_WAVE, speed, color,
                                  BOARD_LED_COLOR_BLACK);
        ...
        int b = atoi(argv[6]);
        board_led_color_t color = {(uint8_t)r, (uint8_t)g, (uint8_t)b};
        board_led_start_animation(BOARD_LED_ANIM_COLOR_WIPE, speed, color,
                                  BOARD_LED_COLOR_BLACK);
        ...
        int b = atoi(argv[6]);
        board_led_color_t color = {(uint8_t)r, (uint8_t)g, (uint8_t)b};
        board_led_start_animation(BOARD_LED_ANIM_SPARKLE, speed, color,
                                  BOARD_LED_COLOR_BLACK);
        printf("Started sparkle animation RGB(%d,%d,%d) (speed: %d)\n", r, g, b,
               speed);
      } else {
        printf("Unknown animation: %s\n", argv[3]);
        printf("Basic animations: rainbow, breathe, chase, fade, wave, "
               "twinkle, fire, pulse, stop\n");
        printf("Creative bands: gradient, rainbow_wave, brightness_wave, "
               "color_wipe, sparkle\n");
      }
      return ESP_OK;
...
static void board_led_console_help(void) {
  ...
  printf(
      "  led board on                          - Turn on all LEDs (white)\n");
  printf("  led board off                         - Turn off all LEDs\n");
  printf("  led board clear                       - Clear all LEDs\n");
  printf(
      "  led board all <R> <G> <B>             - Set all LEDs to RGB color\n");
  printf("  led board set <index> <R> <G> <B>     - Set specific LED to RGB "
         "color\n");
  printf("  led board brightness <0-255>          - Set brightness\n");
  printf("\nAnimation Commands:\n");
  printf("  led board anim stop                   - Stop current animation\n");
  printf("  led board anim rainbow [speed]        - Rainbow animation (speed: "
         "0-255)\n");
  printf(
      "  led board anim breathe <R> <G> <B> [speed] - Breathing animation\n");
  ...
  printf("  led board anim gradient <R1> <G1> <B1> <R2> <G2> <B2> [speed] - "
         "Gradient transition\n");
  printf("  led board anim rainbow_wave [speed]   - Moving rainbow pattern\n");
  printf("  led board anim brightness_wave <R> <G> <B> [speed] - Brightness "
         "wave\n");
  printf(
      "  led board anim color_wipe <R> <G> <B> [speed] - Color wipe effect\n");
  printf(
      "  led board anim sparkle <R> <G> <B> [speed] - Sparkle with fading\n");
  printf("\nConfiguration Commands:\n");
  printf("  led board config save                 - Save current settings to "
         "flash\n");
  printf(
      "  led board config load                 - Load settings from flash\n");
  printf(
      "  led board config reset                - Reset to default settings\n");
  printf("\n");
  printf("Examples:\n");
  ...
  printf(
      "  led board anim gradient 255 0 0 0 0 255 60 - Red to blue gradient\n");
  ...
  printf(
      "  led board config save                 - Save current configuration\n");
  printf(
      "  led board clear                       - Clear LEDs and save state\n");
}