...
static esp_err_t matrix_led_send_event(matrix_led_event_type_t type,
                                       const matrix_led_event_data_t *data);
...
// 图形绘制辅助函数
static void matrix_led_draw_pixel_safe(uint8_t x, uint8_t y,
                                       matrix_led_color_t color);
static void matrix_led_draw_horizontal_line(uint8_t x0, uint8_t x1, uint8_t y,
                                            matrix_led_color_t color);
static void matrix_led_draw_vertical_line(uint8_t x, uint8_t y0, uint8_t y1,
                                          matrix_led_color_t color);

// 色彩校正函数
static esp_err_t matrix_led_apply_all_corrections(matrix_led_color_t color,
                                                  uint8_t brightness,
...
esp_err_t matrix_led_init(void) {
  ...
  if (ret != ESP_OK) {
    ESP_LOGW(TAG, "Failed to initialize color correction: %s",
             esp_err_to_name(ret));
    ...
    if (ret != ESP_OK) {
      ESP_LOGW(TAG, "Failed to register color correction console commands: %s",
               esp_err_to_name(ret));
    }

    // 注册色彩校正配置改变回调
    ret = color_correction_register_change_callback(
        matrix_led_color_correction_changed);
    if (ret != ESP_OK) {
      ESP_LOGW(TAG, "Failed to register color correction change callback: %s",
               esp_err_to_name(ret));
    }
  }
  ...
  s_context.animation_timer = xTimerCreate(
      "matrix_led_timer", pdMS_TO_TICKS(MATRIX_LED_ANIMATION_TASK_DELAY_MS),
      pdTRUE, // 自动重载
      NULL, matrix_led_animation_timer_callback);
  ...
  BaseType_t task_ret = xTaskCreate(
      matrix_led_animation_task, "matrix_led_anim", MATRIX_LED_TASK_STACK_SIZE,
      NULL, MATRIX_LED_TASK_PRIORITY, &s_context.animation_task_handle);
  ...
  // 发送初始化完成事件
  matrix_led_event_data_t event_data = {.type = MATRIX_LED_EVENT_INITIALIZED};
  matrix_led_send_event(MATRIX_LED_EVENT_INITIALIZED, &event_data);

  ESP_LOGI(
      TAG,
      "Matrix LED initialized successfully (GPIO: %d, Size: %dx%d, LEDs: %d)",
...
esp_err_t matrix_led_deinit(void) {
  ...
  if (ret != ESP_OK) {
    ESP_LOGW(TAG, "Failed to deinitialize color correction: %s",
             esp_err_to_name(ret));
  }
...
bool matrix_led_is_initialized(void) { return s_context.initialized; }
...
esp_err_t matrix_led_get_pixel(uint8_t x, uint8_t y,
                               matrix_led_color_t *color) {
...
esp_err_t matrix_led_set_pixels(const matrix_led_pixel_t *pixels,
                                size_t count) {
...
esp_err_t matrix_led_clear(void) {
  ...
  memset(s_context.pixel_buffer, 0,
         MATRIX_LED_COUNT * sizeof(matrix_led_color_t));
...
esp_err_t matrix_led_refresh(void) {
  ...
  for (uint32_t i = 0; i < MATRIX_LED_COUNT; i++) {
    ...
    matrix_led_apply_all_corrections(s_context.pixel_buffer[i],
                                     s_context.brightness, &corrected_color);

    esp_err_t ret =
        led_strip_set_pixel(s_context.led_strip, i, corrected_color.r,
                            corrected_color.g, corrected_color.b);
    if (ret != ESP_OK) {
      xSemaphoreGive(s_context.mutex);
      return ret;
    }
...
esp_err_t matrix_led_draw_line(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1,
                               matrix_led_color_t color) {
  if (!s_context.initialized) {
    return ESP_ERR_INVALID_STATE;
  }
...
esp_err_t matrix_led_draw_rect(const matrix_led_rect_t *rect,
                               matrix_led_color_t color, bool filled) {
  ...
  if (filled) {
    // 填充矩形
    for (uint8_t y = rect->y;
         y < rect->y + rect->height && y < MATRIX_LED_HEIGHT; y++) {
      for (uint8_t x = rect->x;
           x < rect->x + rect->width && x < MATRIX_LED_WIDTH; x++) {
        matrix_led_draw_pixel_safe(x, y, color);
      }
    }
  } else {
    // 绘制矩形边框
    matrix_led_draw_horizontal_line(rect->x, rect->x + rect->width - 1, rect->y,
                                    color);
    matrix_led_draw_horizontal_line(rect->x, rect->x + rect->width - 1,
                                    rect->y + rect->height - 1, color);
    matrix_led_draw_vertical_line(rect->x, rect->y, rect->y + rect->height - 1,
                                  color);
    matrix_led_draw_vertical_line(rect->x + rect->width - 1, rect->y,
                                  rect->y + rect->height - 1, color);
  }
...
esp_err_t matrix_led_draw_circle(uint8_t center_x, uint8_t center_y,
                                 uint8_t radius, matrix_led_color_t color,
                                 bool filled) {
  if (!s_context.initialized) {
    return ESP_ERR_INVALID_STATE;
  }
  ...
  while (x >= y) {
    if (filled) {
      // 绘制水平线填充圆形
      matrix_led_draw_horizontal_line(center_x - x, center_x + x, center_y + y,
                                      color);
      matrix_led_draw_horizontal_line(center_x - x, center_x + x, center_y - y,
                                      color);
      matrix_led_draw_horizontal_line(center_x - y, center_x + y, center_y + x,
                                      color);
      matrix_led_draw_horizontal_line(center_x - y, center_x + y, center_y - x,
                                      color);
    } else {
...
esp_err_t matrix_led_draw_text(uint8_t x, uint8_t y, const char *text,
                               matrix_led_color_t color) {
...
esp_err_t matrix_led_set_brightness(uint8_t brightness) {
  ...
  matrix_led_event_data_t event_data = {
      ...
      .data.brightness_change = {.old_brightness = old_brightness,
                                 .new_brightness = brightness}};
  matrix_led_send_event(MATRIX_LED_EVENT_BRIGHTNESS_CHANGED, &event_data);

  xSemaphoreGive(s_context.mutex);

  ESP_LOGI(TAG, "Brightness changed from %d%% to %d%%", old_brightness,
           brightness);
...
esp_err_t matrix_led_set_mode(matrix_led_mode_t mode) {
  ...
  matrix_led_event_data_t event_data = {
      ...
      .data.mode_change = {.old_mode = old_mode, .new_mode = mode}};
...
esp_err_t
matrix_led_play_animation(matrix_led_animation_type_t animation_type,
                          const matrix_led_animation_config_t *config) {
  if (!s_context.initialized) {
    return ESP_ERR_INVALID_STATE;
  }
  ...
  if (config != NULL) {
    ...
    // 设置默认配置
    memset(&s_context.animation.config, 0,
           sizeof(matrix_led_animation_config_t));
    ...
    snprintf(s_context.animation.config.name, MATRIX_LED_MAX_NAME_LEN,
             "Anim_%d", animation_type);
  }
  ...
  xTimerChangePeriod(s_context.animation_timer,
                     pdMS_TO_TICKS(s_context.animation.config.frame_delay_ms),
                     0);
  ...
  matrix_led_event_data_t event_data = {
      ...
      .data.animation = {.animation_name = {0}}};
  strncpy(event_data.data.animation.animation_name,
          s_context.animation.config.name, MATRIX_LED_MAX_NAME_LEN - 1);
  matrix_led_send_event(MATRIX_LED_EVENT_ANIMATION_STARTED, &event_data);

  xSemaphoreGive(s_context.mutex);

  ESP_LOGI(TAG, "Animation started: %s (type: %d)",
           s_context.animation.config.name, animation_type);
...
esp_err_t matrix_led_stop_animation(void) {
  ...
  strncpy(animation_name, s_context.animation.config.name,
          sizeof(animation_name));
  ...
  matrix_led_event_data_t event_data = {
      ...
      .data.animation = {.animation_name = {0}}};
  strncpy(event_data.data.animation.animation_name, animation_name,
          MATRIX_LED_MAX_NAME_LEN - 1);
...
esp_err_t matrix_led_load_animation_from_file(const char *filename,
...
esp_err_t matrix_led_play_custom_animation(const char *animation_name) {
  ...
  // TODO: 实现播放自定义动画的功能
  ESP_LOGW(TAG, "Play custom animation not implemented yet: %s",
           animation_name);
...
esp_err_t matrix_led_show_test_pattern(void) {
  ...
  // 绘制对角线（绿色）
  matrix_led_draw_line(0, 0, MATRIX_LED_WIDTH - 1, MATRIX_LED_HEIGHT - 1,
                       MATRIX_LED_COLOR_GREEN);
  matrix_led_draw_line(0, MATRIX_LED_HEIGHT - 1, MATRIX_LED_WIDTH - 1, 0,
                       MATRIX_LED_COLOR_GREEN);

  // 绘制中心十字（蓝色）
  uint8_t center_x = MATRIX_LED_WIDTH / 2;
  uint8_t center_y = MATRIX_LED_HEIGHT / 2;
  matrix_led_draw_line(center_x, 0, center_x, MATRIX_LED_HEIGHT - 1,
                       MATRIX_LED_COLOR_BLUE);
  matrix_led_draw_line(0, center_y, MATRIX_LED_WIDTH - 1, center_y,
                       MATRIX_LED_COLOR_BLUE);
...
esp_err_t matrix_led_breathe_effect(matrix_led_color_t color, uint8_t speed) {
  ...
  // 设置呼吸动画配置
  matrix_led_animation_config_t config = {.type = MATRIX_LED_ANIM_BREATHE,
                                          .duration_ms = 0, // 无限循环
...
esp_err_t matrix_led_color_interpolate(matrix_led_color_t color1,
                                       matrix_led_color_t color2, float ratio,
                                       matrix_led_color_t *result) {
...
esp_err_t matrix_led_apply_brightness(matrix_led_color_t color,
                                      uint8_t brightness,
                                      matrix_led_color_t *result) {
...
static esp_err_t matrix_led_apply_all_corrections(matrix_led_color_t color,
  ...
  esp_err_t ret =
      matrix_led_apply_brightness(color, brightness, &brightness_adjusted);
  if (ret != ESP_OK) {
    return ret;
  }
  ...
  if (color_correction_is_enabled()) {
    ...
    color_rgb_t input = {.r = brightness_adjusted.r,
...
esp_err_t matrix_led_save_config(void) {
  ...
  ret = config_manager_set(MATRIX_LED_CONFIG_NAMESPACE,
                           MATRIX_LED_CONFIG_KEY_BRIGHTNESS, CONFIG_TYPE_UINT8,
  ...
  ret = config_manager_set(MATRIX_LED_CONFIG_NAMESPACE,
                           MATRIX_LED_CONFIG_KEY_MODE, CONFIG_TYPE_UINT8, &mode,
                           sizeof(mode));
  if (ret != ESP_OK) {
  ...
  ret = config_manager_set(MATRIX_LED_CONFIG_NAMESPACE,
                           MATRIX_LED_CONFIG_KEY_ENABLE, CONFIG_TYPE_UINT8,
  ...
  if (s_context.animation.is_running) {
    ...
    ret = config_manager_set(MATRIX_LED_CONFIG_NAMESPACE, "anim_type",
                             CONFIG_TYPE_UINT8, &anim_type, sizeof(anim_type));
    if (ret != ESP_OK) {
    ...
    ret =
        config_manager_set(MATRIX_LED_CONFIG_NAMESPACE, "anim_speed",
                           CONFIG_TYPE_UINT8, &anim_speed, sizeof(anim_speed));
    if (ret != ESP_OK) {
    ...
    ret = config_manager_set(
        MATRIX_LED_CONFIG_NAMESPACE, MATRIX_LED_CONFIG_KEY_ANIMATION,
        CONFIG_TYPE_STRING, s_context.animation.config.name, str_len);
    if (ret != ESP_OK) {
      ESP_LOGE(TAG, "Failed to save animation config: %s",
               esp_err_to_name(ret));
      return ret;
    }
    ...
    config_manager_delete(MATRIX_LED_CONFIG_NAMESPACE,
                          MATRIX_LED_CONFIG_KEY_ANIMATION);
    ...
    if (s_context.pixel_buffer != NULL) {
      ...
      ret = config_manager_set(
          MATRIX_LED_CONFIG_NAMESPACE, MATRIX_LED_CONFIG_KEY_STATIC_DATA,
          CONFIG_TYPE_BLOB, s_context.pixel_buffer, data_size);
      if (ret == ESP_OK) {
        ESP_LOGI(TAG, "Static LED data saved (%zu bytes)", data_size);
      } else {
        ESP_LOGW(TAG, "Failed to save static LED data: %s",
                 esp_err_to_name(ret));
      }
    }
  }
...
esp_err_t matrix_led_load_config(void) {
  ...
  ret = config_manager_get(MATRIX_LED_CONFIG_NAMESPACE,
                           MATRIX_LED_CONFIG_KEY_BRIGHTNESS, CONFIG_TYPE_UINT8,
                           &value_u8, &value_size);
  if (ret == ESP_OK) {
  ...
  ret = config_manager_get(MATRIX_LED_CONFIG_NAMESPACE,
                           MATRIX_LED_CONFIG_KEY_MODE, CONFIG_TYPE_UINT8,
                           &value_u8, &mode_size);
  ...
  ret = config_manager_get(MATRIX_LED_CONFIG_NAMESPACE,
                           MATRIX_LED_CONFIG_KEY_ANIMATION, CONFIG_TYPE_STRING,
                           NULL, &required_size);
  if (ret == ESP_OK && required_size > 0) {
    animation_name = malloc(required_size);
    if (animation_name != NULL) {
      ret = config_manager_get(
          MATRIX_LED_CONFIG_NAMESPACE, MATRIX_LED_CONFIG_KEY_ANIMATION,
      ...
      if (ret == ESP_OK) {
        ...
        ret = config_manager_get(MATRIX_LED_CONFIG_NAMESPACE, "anim_type",
                                 CONFIG_TYPE_UINT8, &anim_type_u8, &type_size);
        if (ret == ESP_OK) {
        ...
        config_manager_get(MATRIX_LED_CONFIG_NAMESPACE, "anim_speed",
                           CONFIG_TYPE_UINT8, &saved_anim_speed, &speed_size);
  ...
  if (!s_context.enabled) {
    ...
    if (saved_anim_type < sizeof(anim_names) / sizeof(anim_names[0])) {
      snprintf(config.name, MATRIX_LED_MAX_NAME_LEN, "%s_%d",
               anim_names[saved_anim_type], saved_anim_speed);
    }
    ...
    if (anim_ret == ESP_OK) {
      ...
      ESP_LOGE(TAG, "Failed to restore animation: %s",
    ...
    ret = config_manager_get(MATRIX_LED_CONFIG_NAMESPACE,
                             MATRIX_LED_CONFIG_KEY_STATIC_DATA,
                             CONFIG_TYPE_BLOB, NULL, &static_data_size);
    if (ret == ESP_OK &&
        static_data_size == MATRIX_LED_COUNT * sizeof(matrix_led_color_t)) {
      // 恢复静态像素数据
      ret = config_manager_get(
          MATRIX_LED_CONFIG_NAMESPACE, MATRIX_LED_CONFIG_KEY_STATIC_DATA,
      ...
      if (ret == ESP_OK) {
        matrix_led_refresh();
        ESP_LOGI(TAG, "Static LED content restored (%zu bytes)",
                 static_data_size);
      } else {
        matrix_led_clear();
        matrix_led_refresh();
      ...
      ESP_LOGI(
          TAG,
          "Matrix LED cleared (no animation or static content to restore)");
    }
  }

  ESP_LOGI(TAG,
           "Matrix LED configuration loaded (brightness: %d%%, mode: %d, "
           "enabled: %s)",
           s_context.brightness, s_context.mode,
           s_context.enabled ? "yes" : "no");
...
static esp_err_t matrix_led_init_hardware(void) {
  ...
  // LED条带配置
  led_strip_config_t strip_config = {.strip_gpio_num = MATRIX_LED_GPIO,
  ...
  // RMT后端配置 - Matrix LED需要更多资源用于1024个LED
  led_strip_rmt_config_t rmt_config = {
      .clk_src = RMT_CLK_SRC_DEFAULT,
      .resolution_hz = MATRIX_LED_RMT_RESOLUTION,
      .mem_block_symbols = 96, // 为Matrix LED分配更多内存(2个通道的内存)
      .flags = {
  ...
  if (ret != ESP_OK) {
    ...
    ESP_LOGE(
        TAG,
        "This may be due to RMT resource conflicts with other LED components");
    ESP_LOGE(TAG,
             "Try adjusting initialization order or RMT channel allocation");
    return ret;
  }
...
static void matrix_led_animation_task(void *pvParameters) {
  ...
  while (s_context.initialized) {
    // 等待动画信号量或超时
    if (xSemaphoreTake(s_context.animation_semaphore, pdMS_TO_TICKS(1000)) ==
        pdTRUE) {
      // 执行动画更新
...
static void matrix_led_animation_timer_callback(TimerHandle_t xTimer) {
  ...
  xSemaphoreGiveFromISR(s_context.animation_semaphore,
...
static esp_err_t matrix_led_validate_coordinates(uint8_t x, uint8_t y) {
  // 添加防护检查，避免在组件未初始化时被调用
  if (!s_context.initialized) {
    ESP_LOGE(TAG,
             "matrix_led_validate_coordinates called before initialization!");
    return ESP_ERR_INVALID_STATE;
  }
...
static uint32_t matrix_led_xy_to_index(uint8_t x, uint8_t y) {
  // 添加防护检查，避免在组件未初始化时被调用
  if (!s_context.initialized) {
    ESP_LOGE(TAG, "matrix_led_xy_to_index called before initialization!");
    return 0; // 返回安全的默认值
  }

  // 添加边界检查
  if (x >= MATRIX_LED_WIDTH || y >= MATRIX_LED_HEIGHT) {
    ESP_LOGE(TAG, "matrix_led_xy_to_index: invalid coordinates (%d, %d)", x, y);
    return 0; // 返回安全的默认值
  }
...
matrix_led_index_to_xy(uint32_t index, uint8_t *x, uint8_t *y) {
...
static esp_err_t matrix_led_send_event(matrix_led_event_type_t type,
  ...
  return event_manager_post_event(MATRIX_LED_EVENTS, type, (void *)data,
                                  sizeof(matrix_led_event_data_t),
                                  portMAX_DELAY);
}
...
static void matrix_led_animate_rainbow(void) {
  uint32_t time_offset =
      (xTaskGetTickCount() - s_context.animation.start_time) *
      s_context.animation.config.speed / 10;

  for (uint8_t y = 0; y < MATRIX_LED_HEIGHT; y++) {
    for (uint8_t x = 0; x < MATRIX_LED_WIDTH; x++) {
      uint16_t hue = ((x + y) * 360 / (MATRIX_LED_WIDTH + MATRIX_LED_HEIGHT) +
                      time_offset) %
                     360;
...
static void matrix_led_animate_wave(void) {
  uint32_t time_offset =
      (xTaskGetTickCount() - s_context.animation.start_time) *
      s_context.animation.config.speed / 20;
  ...
  for (uint8_t y = 0; y < MATRIX_LED_HEIGHT; y++) {
    for (uint8_t x = 0; x < MATRIX_LED_WIDTH; x++) {
      ...
      if (matrix_led_color_interpolate(secondary, primary, wave, &result) ==
          ESP_OK) {
        uint32_t index = matrix_led_xy_to_index(x, y);
...
static void matrix_led_animate_breathe(void) {
  ...
  if (matrix_led_apply_brightness(base_color, (uint8_t)(breathe * 100),
                                  &result) == ESP_OK) {
...
static void matrix_led_animate_rotate(void) {
  ...
  uint32_t time_offset =
      (xTaskGetTickCount() - s_context.animation.start_time) *
      s_context.animation.config.speed / 30;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (x < MATRIX_LED_WIDTH && y < MATRIX_LED_HEIGHT) {
      matrix_led_draw_pixel_safe(x, y,
                                 s_context.animation.config.primary_color);
    }
  }
}
...
static void matrix_led_animate_fade(void) {
  uint32_t time_offset =
      (xTaskGetTickCount() - s_context.animation.start_time) *
      s_context.animation.config.speed / 40;
...
static void matrix_led_draw_horizontal_line(uint8_t x0, uint8_t x1, uint8_t y,
                                            matrix_led_color_t color) {
  if (y >= MATRIX_LED_HEIGHT)
    return;

  uint8_t start_x = (x0 < x1) ? x0 : x1;
  uint8_t end_x = (x0 < x1) ? x1 : x0;

  if (end_x >= MATRIX_LED_WIDTH)
    end_x = MATRIX_LED_WIDTH - 1;
...
static void matrix_led_draw_vertical_line(uint8_t x, uint8_t y0, uint8_t y1,
                                          matrix_led_color_t color) {
  if (x >= MATRIX_LED_WIDTH)
    return;

  uint8_t start_y = (y0 < y1) ? y0 : y1;
  uint8_t end_y = (y0 < y1) ? y1 : y0;

  if (end_y >= MATRIX_LED_HEIGHT)
    end_y = MATRIX_LED_HEIGHT - 1;
...
int matrix_led_cmd_handler(int argc, char **argv) {
  ...
  if (argc < 1 || (argc >= 1 && strcmp(argv[0], "matrix") != 0)) {
    ...
    printf(
        "  led matrix draw line <x0> <y0> <x1> <y1> <r> <g> <b> - Draw line\n");
    printf("  led matrix draw rect <x> <y> <w> <h> <r> <g> <b> [fill] - Draw "
           "rectangle\n");
    printf("  led matrix draw circle <x> <y> <radius> <r> <g> <b> [fill] - "
           "Draw circle\n");
    printf("Configuration:\n");
    printf("  led matrix config <save|load|reset|export|import> - Config "
           "management\n");
    printf("Storage Features:\n");
    printf("  led matrix image <export|import> <filepath> - Image save/load\n");
    printf("  led matrix storage status             - Check storage "
           "availability\n");
  ...
  if (strcmp(argv[1], "help") == 0) {
    ...
    printf("\nStorage Features:\n");
    printf("  led matrix image export <file>     - Save current display "
           "(points format)\n");
    printf("  led matrix image import <file> [name] - Load image from file "
           "(optionally by name)\n");
    printf(
        "  led matrix image list <file>       - List all animations in file\n");
    ...
    if (had_animation) {
      ...
      if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Failed to stop animation before clear: %s",
                 esp_err_to_name(ret));
      }
    }
    ...
    if (strcmp(argv[2], "status") == 0) {
      ...
      const char *test_paths[] = {"/sdcard/cfg.txt", "/sdcard/led/cfg.txt",
                                  "/sdcard/cfg.json", "/sdcard/led/cfg.json",
                                  "/sdcard/config.txt"};
      ...
      const char *test_paths[] = {"/sdcard/matrix_led/test.txt",
                                  "/sdcard/led/test.txt", "/sdcard/test.txt"};
      ...
      if (info_ret == ESP_OK) {
        ...
        printf("  Used space: %lu MB (%.1f%%)\n",
               ...
               total_mb > 0 ? (float)(total_mb - free_mb) / total_mb * 100.0
                            : 0.0);
      } else {
    ...
    if (strcmp(argv[2], "line") == 0) {
      ...
      if (argc < 9) {
        printf(
            "Usage: led matrix draw rect <x> <y> <w> <h> <r> <g> <b> [fill]\n");
        return 1;
      }
      ...
      if (argc < 8) {
        printf("Usage: led matrix draw circle <x> <y> <radius> <r> <g> <b> "
               "[fill]\n");
        return 1;
      }
      int x = atoi(argv[3]), y = atoi(argv[4]);
...
static void matrix_led_register_console_commands(void) {
  ...
  ESP_LOGI(TAG,
           "Matrix LED uses 'led matrix' commands (shared with touch LED)");
...
esp_err_t matrix_led_export_config(const char *filepath) {
  ...
  if (last_slash != NULL && last_slash != filepath &&
      strstr(filepath, "/sdcard/") == filepath) {
    // 检查是否不在根目录，如果不在根目录则尝试创建
    if (strncmp(filepath, "/sdcard/", 8) == 0 &&
        strchr(filepath + 8, '/') != NULL) {
      ESP_LOGI(TAG, "Attempting to create directory structure for: %s",
               filepath);
      esp_err_t dir_ret = ensure_directory_exists(filepath);
      if (dir_ret != ESP_OK) {
        ESP_LOGW(TAG, "Failed to create directory, please use root directory "
                      "like /sdcard/config.json");
        return ESP_ERR_INVALID_ARG;
      }
    }
  }
  ...
  if (s_context.animation.is_running) {
    ...
    cJSON_AddNumberToObject(animation, "speed",
                            s_context.animation.config.speed);
  ...
  if (file == NULL) {
    ...
    if (file != NULL) {
      ...
      ESP_LOGE(TAG, "Simple text write also failed (errno: %d - %s)", errno,
               strerror(errno));
    }
    return ESP_FAIL;
  }
  ...
  // 强制刷新到磁盘
  if (fflush(file) != 0) {
    ESP_LOGW(TAG, "Failed to flush file buffer (errno: %d - %s)", errno,
             strerror(errno));
  }
  ...
  if (written == json_len) {
    ESP_LOGI(TAG, "Configuration exported successfully to: %s (%zu bytes)",
             filepath, written);
    return ESP_OK;
  } else {
    ESP_LOGE(TAG,
             "Failed to write complete configuration to file (wrote %zu of %zu "
             "bytes)",
             written, json_len);
    return ESP_FAIL;
  }
  ...
  if (animation != NULL) {
    ...
    if (cJSON_IsNumber(anim_type) && cJSON_IsNumber(anim_speed)) {
      ...
      if (cJSON_IsString(anim_name)) {
        strncpy(config.name, anim_name->valuestring,
                MATRIX_LED_MAX_NAME_LEN - 1);
      }
...
esp_err_t matrix_led_export_image(const char *filepath) {
  ...
  // 添加动画名称（基于当前时间戳）
  char name_buffer[64];
  snprintf(name_buffer, sizeof(name_buffer), "Static_Image_%lu",
...
esp_err_t matrix_led_import_image_by_name(const char *filepath,
                                          const char *animation_name) {
  if (!s_context.initialized) {
    return ESP_ERR_INVALID_STATE;
  }
        ...
        for (int i = 0; i < point_count; i++) {
          ...
          if (point != NULL) {
            ...
            if (cJSON_IsNumber(x) && cJSON_IsNumber(y) && cJSON_IsNumber(r) &&
              ...
              // 检查坐标范围
              if (px >= 0 && px < MATRIX_LED_WIDTH && py >= 0 &&
                  py < MATRIX_LED_HEIGHT) {
  ...
  if (!loaded_data && animation_name == NULL) {
    ...
    if (cJSON_IsNumber(width) && cJSON_IsNumber(height) &&
        cJSON_IsArray(pixels) && width->valueint == MATRIX_LED_WIDTH &&
        height->valueint == MATRIX_LED_HEIGHT) {
  ...
  if (!loaded_data) {
    if (animation_name != NULL) {
      ESP_LOGE(TAG, "Animation '%s' not found or no valid data",
               animation_name);
    } else {
...
esp_err_t matrix_led_export_animation(const char *animation_name,
                                      const char *filepath) {
  if (!s_context.initialized) {
    return ESP_ERR_INVALID_STATE;
  }
...
esp_err_t matrix_led_import_animation(const char *filepath,
                                      const char *animation_name) {
  if (!s_context.initialized) {
    return ESP_ERR_INVALID_STATE;
  }
