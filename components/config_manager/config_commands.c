...
static esp_err_t confirm_dangerous_operation(const char *operation, const char *confirm_text);
static bool is_valid_namespace_name(const char *name);
static bool is_valid_key_name(const char *name);

// NVS data helpers
static esp_err_t show_nvs_key_value(nvs_handle_t handle, const char *namespace, const char *key);
static esp_err_t show_all_namespace_keys(const char *namespace);
static esp_err_t detect_and_show_value_type(nvs_handle_t handle, const char *key);
...
esp_err_t config_manager_register_commands(void)
{
    console_cmd_t config_command = {
        .command = "config",
        .help = "config <namespace|data|backup|system|help> [args...] - Configuration management",
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to register config command: %s", esp_err_to_name(ret));
        return ret;
    }
...
static esp_err_t cmd_config_namespace(int argc, char **argv)
{
    if (argc < 2) {
        ...
        printf("  delete <namespace>      - Delete namespace (requires confirmation)\n");
        return ESP_OK;
    }
...
static esp_err_t cmd_config_data(int argc, char **argv)
{
    if (argc < 2) {
        ...
        printf("  dump <namespace> <key>           - Show detailed hex dump of blob data\n");
        printf("\n");
        printf("Supported types: u8, u16, u32, i8, i16, i32, str, bool, blob\n");
        printf("Note: String values cannot contain spaces. Use underscores instead.\n");
...
{
    if (argc < 2) {
        printf("Usage: config backup <create|restore|list> [args...]\n");
        printf("Commands:\n");
        printf("  create [namespace] [file]  - Create configuration backup\n");
    ...
    if (strcmp(action, "create") == 0) {
        printf("Backup functionality will be implemented in Phase 2\n");
        return ESP_OK;
    } else if (strcmp(action, "restore") == 0) {
        printf("Restore functionality will be implemented in Phase 2\n");
        return ESP_OK;
    } else if (strcmp(action, "list") == 0) {
        printf("Backup list functionality will be implemented in Phase 2\n");
        return ESP_OK;
    } else {
...
static esp_err_t cmd_config_help(void)
{
    ...
    printf("\n");
    printf("OVERVIEW\n");
    printf("  The config command provides comprehensive management of NVS-based\n");
    ...
    printf("    delete <ns>             Delete namespace (requires confirmation)\n");
    printf("\n");
    ...
    printf("\n");
    printf("  backup - Backup and Restore (Phase 2)\n");
    printf("    create [ns] [file]      Create configuration backup\n");
    printf("    restore <file>          Restore from backup file\n");
    printf("    list                    List available backup files\n");
    printf("\n");
...
static esp_err_t config_namespace_list(void)
{
    ...
    esp_err_t ret = nvs_entry_find(NVS_DEFAULT_PART_NAME, NULL, NVS_TYPE_ANY, &iterator);
    ...
    for (int i = 0; i < namespace_count; i++) {
        ...
        int key_count = 0;
        ret = nvs_entry_find(NVS_DEFAULT_PART_NAME, namespaces[i], NVS_TYPE_ANY, &ns_iterator);
...
static esp_err_t config_namespace_stats(const char *namespace)
{
    ...
    nvs_iterator_t iterator = NULL;
    ret = nvs_entry_find(NVS_DEFAULT_PART_NAME, namespace, NVS_TYPE_ANY, &iterator);
    ...
    printf("\nKey Types:\n");
    if (counts[0] > 0) printf("  8-bit integers:  %d\n", counts[0]);
    if (counts[1] > 0) printf("  16-bit integers: %d\n", counts[1]);
    if (counts[2] > 0) printf("  32-bit integers: %d\n", counts[2]);
    if (counts[3] > 0) printf("  64-bit integers: %d\n", counts[3]);
    if (counts[4] > 0) printf("  Strings:         %d\n", counts[4]);
    if (counts[5] > 0) printf("  Blobs:           %d\n", counts[5]);
    if (counts[6] > 0) printf("  Other types:     %d\n", counts[6]);
...
static esp_err_t config_data_delete(const char *namespace, const char *key)
{
    ...
    // For now, auto-confirm for testing - in full implementation we'd wait for user input
    printf("y\n");
...
static esp_err_t config_data_list(const char *namespace)
{
    ...
    nvs_iterator_t iterator = NULL;
    esp_err_t ret = nvs_entry_find(NVS_DEFAULT_PART_NAME, namespace, NVS_TYPE_ANY, &iterator);
    ...
    while (iterator != NULL) {
        ...
        switch (info.type) {
            case NVS_TYPE_U8: type_name = "u8"; break;
            case NVS_TYPE_I8: type_name = "i8"; break;
            case NVS_TYPE_U16: type_name = "u16"; break;
            case NVS_TYPE_I16: type_name = "i16"; break;
            case NVS_TYPE_U32: type_name = "u32"; break;
            case NVS_TYPE_I32: type_name = "i32"; break;
            case NVS_TYPE_U64: type_name = "u64"; break;
            case NVS_TYPE_I64: type_name = "i64"; break;
            case NVS_TYPE_STR: type_name = "str"; break;
            case NVS_TYPE_BLOB: type_name = "blob"; break;
...
static esp_err_t config_system_info(void)
{
    ...
    printf("Note: Detailed partition info requires ESP partition API integration\n");
...
static config_type_t parse_type_string(const char *type_str)
{
    if (strcmp(type_str, "u8") == 0) return CONFIG_TYPE_U8;
    if (strcmp(type_str, "u16") == 0) return CONFIG_TYPE_U16;
    if (strcmp(type_str, "u32") == 0) return CONFIG_TYPE_U32;
    if (strcmp(type_str, "i8") == 0) return CONFIG_TYPE_I8;
    if (strcmp(type_str, "i16") == 0) return CONFIG_TYPE_I16;
    if (strcmp(type_str, "i32") == 0) return CONFIG_TYPE_I32;
    if (strcmp(type_str, "str") == 0) return CONFIG_TYPE_STRING;
    if (strcmp(type_str, "bool") == 0) return CONFIG_TYPE_BOOL;
    if (strcmp(type_str, "blob") == 0) return CONFIG_TYPE_BLOB;
...
{
    switch (type) {
        case CONFIG_TYPE_U8: return "u8";
        case CONFIG_TYPE_U16: return "u16";
        case CONFIG_TYPE_U32: return "u32";
        case CONFIG_TYPE_I8: return "i8";
        case CONFIG_TYPE_I16: return "i16";
        case CONFIG_TYPE_I32: return "i32";
        case CONFIG_TYPE_STRING: return "str";
        case CONFIG_TYPE_BOOL: return "bool";
        case CONFIG_TYPE_BLOB: return "blob";
