...
esp_err_t touch_led_init(const touch_led_config_t *config)
{
    ...
    esp_err_t ret = led_strip_new_rmt_device(&strip_config, &rmt_config, &s_touch_led.led_strip);
    if (ret != ESP_OK) {
    ...
    if (config->touch_gpio != GPIO_NUM_NC) {
        ...
        gpio_config_t touch_gpio_config = {
            .pin_bit_mask = (1ULL << config->touch_gpio),
    ...
    ESP_LOGI(TAG, "Touch LED initialized with %d LEDs on GPIO %d, touch on GPIO %d", 
...
esp_err_t touch_led_deinit(void)
{
    ...
    // Reset touch GPIO
    if (s_touch_led.touch_enabled && s_touch_led.config.touch_gpio != GPIO_NUM_NC) {
...
esp_err_t touch_led_set_all_color(rgb_color_t color)
{
    ...
    for (uint16_t i = 0; i < s_touch_led.config.led_count; i++) {
        esp_err_t err = led_strip_set_pixel(s_touch_led.led_strip, i, 
                                           color.red, color.green, color.blue);
        if (err != ESP_OK) {
            ret = err;
        }
...
esp_err_t touch_led_set_touch_threshold(uint32_t threshold)
{
    ...
    // Note: For GPIO-based touch detection, threshold is stored but not actively used
    // In a real implementation, you might configure ADC thresholds here
    xSemaphoreGive(s_touch_led.mutex);
    ...
    while (s_touch_led.animation_running) {
        ...
        switch (s_touch_led.current_animation) {
            case TOUCH_LED_ANIM_RAINBOW: {
                ...
                for (uint16_t i = 0; i < s_touch_led.config.led_count; i++) {
                    float led_hue = fmodf(hue + (i * 360.0f / s_touch_led.config.led_count), 360.0f);
                    rgb_color_t led_color = hsv_to_rgb(led_hue, 1.0f, 1.0f);
                    apply_brightness(&led_color, s_touch_led.current_brightness);
                    led_strip_set_pixel(s_touch_led.led_strip, i, 
            ...
            case TOUCH_LED_ANIM_BREATHE: {
                float brightness_factor = (sinf(s_touch_led.animation_step * 2.0f * M_PI / 255.0f) + 1.0f) / 2.0f;
                rgb_color_t color = s_touch_led.animation_primary_color;
                uint8_t brightness = (uint8_t)(s_touch_led.current_brightness * brightness_factor);
            ...
            case TOUCH_LED_ANIM_FADE: {
                ...
                uint8_t brightness = (s_touch_led.animation_step > 127) ? 
                                   (255 - s_touch_led.animation_step) : s_touch_led.animation_step;
...
static void touch_detection_task(void *arg)
{
    ...
    while (s_touch_led.touch_enabled) {
        ...
        bool raw_touch = s_touch_led.config.touch_invert ? (gpio_level == 0) : (gpio_level == 1);
        ...
        if (current_touch != last_touch_state) {
            ...
            if (xSemaphoreTake(s_touch_led.mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
                ...
                touch_event_callback_t callback = s_touch_led.event_callback; // Store callback locally
                xSemaphoreGive(s_touch_led.mutex);
                ...
                if (callback) {
                    if (current_touch) {
                        ...
                        // Touch release
                        uint32_t duration = (esp_timer_get_time() / 1000) - press_start_time;
                        if (duration > 1000) {
...
{
    if (strcasecmp(color_str, "red") == 0) return TOUCH_LED_COLOR_RED;
    if (strcasecmp(color_str, "green") == 0) return TOUCH_LED_COLOR_GREEN;
    if (strcasecmp(color_str, "blue") == 0) return TOUCH_LED_COLOR_BLUE;
    if (strcasecmp(color_str, "white") == 0) return TOUCH_LED_COLOR_WHITE;
    if (strcasecmp(color_str, "yellow") == 0) return TOUCH_LED_COLOR_YELLOW;
    if (strcasecmp(color_str, "cyan") == 0) return TOUCH_LED_COLOR_CYAN;
    if (strcasecmp(color_str, "magenta") == 0) return TOUCH_LED_COLOR_MAGENTA;
    if (strcasecmp(color_str, "orange") == 0) return TOUCH_LED_COLOR_ORANGE;
    if (strcasecmp(color_str, "purple") == 0) return TOUCH_LED_COLOR_PURPLE;
    if (strcasecmp(color_str, "off") == 0) return TOUCH_LED_COLOR_OFF;
...
{
    if (memcmp(&color, &TOUCH_LED_COLOR_RED, sizeof(rgb_color_t)) == 0) return "Red";
    if (memcmp(&color, &TOUCH_LED_COLOR_GREEN, sizeof(rgb_color_t)) == 0) return "Green";
    if (memcmp(&color, &TOUCH_LED_COLOR_BLUE, sizeof(rgb_color_t)) == 0) return "Blue";
    if (memcmp(&color, &TOUCH_LED_COLOR_WHITE, sizeof(rgb_color_t)) == 0) return "White";
    if (memcmp(&color, &TOUCH_LED_COLOR_YELLOW, sizeof(rgb_color_t)) == 0) return "Yellow";
    if (memcmp(&color, &TOUCH_LED_COLOR_CYAN, sizeof(rgb_color_t)) == 0) return "Cyan";
    if (memcmp(&color, &TOUCH_LED_COLOR_MAGENTA, sizeof(rgb_color_t)) == 0) return "Magenta";
    if (memcmp(&color, &TOUCH_LED_COLOR_ORANGE, sizeof(rgb_color_t)) == 0) return "Orange";
    if (memcmp(&color, &TOUCH_LED_COLOR_PURPLE, sizeof(rgb_color_t)) == 0) return "Purple";
    if (memcmp(&color, &TOUCH_LED_COLOR_OFF, sizeof(rgb_color_t)) == 0) return "Off";
    return "Custom";
}
...
{
    switch (anim) {
        case TOUCH_LED_ANIM_NONE: return "None";
        case TOUCH_LED_ANIM_FADE: return "Fade";
        case TOUCH_LED_ANIM_RAINBOW: return "Rainbow";
        case TOUCH_LED_ANIM_BREATHE: return "Breathe";
        case TOUCH_LED_ANIM_PULSE: return "Pulse";
        case TOUCH_LED_ANIM_WAVE: return "Wave";
        case TOUCH_LED_ANIM_SPARKLE: return "Sparkle";
...
static esp_err_t cmd_led_set(int argc, char **argv)
{
    if (argc < 2) {
        ...
        printf("  color: red|green|blue|white|yellow|cyan|magenta|orange|purple|off|RRGGBB\n");
...
{
    if (argc < 2) {
        printf("Usage: led animation <start|stop> [mode] [speed] [color1] [color2]\n");
        ...
        printf("  color1: primary color (red|green|blue|white|yellow|cyan|magenta|orange|purple|off|RRGGBB)\n");
    ...
    if (strcasecmp(argv[1], "start") == 0) {
        ...
        if (strcasecmp(argv[2], "fade") == 0) animation = TOUCH_LED_ANIM_FADE;
        else if (strcasecmp(argv[2], "rainbow") == 0) animation = TOUCH_LED_ANIM_RAINBOW;
        else if (strcasecmp(argv[2], "breathe") == 0) animation = TOUCH_LED_ANIM_BREATHE;
        else if (strcasecmp(argv[2], "pulse") == 0) animation = TOUCH_LED_ANIM_PULSE;
        else if (strcasecmp(argv[2], "wave") == 0) animation = TOUCH_LED_ANIM_WAVE;
        else if (strcasecmp(argv[2], "sparkle") == 0) animation = TOUCH_LED_ANIM_SPARKLE;
        else {
        ...
        rgb_color_t secondary_color = TOUCH_LED_COLOR_BLUE; // Default secondary color
        if (argc >= 6) {
            secondary_color = parse_color(argv[5]);
        }

        esp_err_t ret = touch_led_start_animation(animation, speed, primary_color, secondary_color);
        if (ret == ESP_OK) {
...
static int cmd_led_help(int argc, char **argv)
{
    ...
    printf("  led touch status                  - Show LED status and configuration\n");
    printf("  led touch help                    - Show this help information\n");
    
    printf("\n🎨 LED Color Control:\n");
    printf("  led touch set <color>             - Set LED color (single LED only)\n");
    printf("    color: red|green|blue|white|yellow|cyan|magenta|orange|purple|off|RRGGBB\n");
    ...
    printf("  led touch sensor threshold <val>  - Set touch threshold (0-4095)\n");
    
    printf("\n� Configuration Management:\n");
    printf("  led touch config save             - Save current settings to memory\n");
    ...
    printf("  led touch animation start breathe 50 green - Slow green breathing\n");
    printf("  led touch sensor threshold 800    - Set touch sensitivity\n");
    
    printf("\n🎨 Available Colors:\n");
    printf("  red, green, blue, white, yellow, cyan, magenta, orange, purple, off\n");
...
static esp_err_t cmd_led_touch(int argc, char **argv)
{
    if (argc < 2) {
        ...
        printf("Available commands: status|set|brightness|clear|animation|sensor|config|help\n");
...
esp_err_t touch_led_save_config(void)
{
    ...
    touch_led_saved_config_t saved_config = {
        ...
        .static_color = s_touch_led.has_static_color ? s_touch_led.current_static_color : (rgb_color_t){0, 0, 0},
    ...
    esp_err_t ret = config_manager_set(TOUCH_LED_CONFIG_NAMESPACE, TOUCH_LED_CONFIG_KEY,
                                      CONFIG_TYPE_BLOB, &saved_config, sizeof(saved_config));
    
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "Touch LED configuration saved (brightness: %d, animation: %d, running: %s)",
...
esp_err_t touch_led_load_config(void)
{
    ...
    esp_err_t ret = config_manager_get(TOUCH_LED_CONFIG_NAMESPACE, TOUCH_LED_CONFIG_KEY,
                                      CONFIG_TYPE_BLOB, &saved_config, &config_size);
    
    if (ret == ESP_OK) {
        // Check version compatibility
        if (saved_config.version != TOUCH_LED_CONFIG_VERSION) {
            ESP_LOGW(TAG, "Configuration version mismatch (saved: %d, current: %d), using defaults",
        ...
        if (saved_config.animation_running && saved_config.animation != TOUCH_LED_ANIM_NONE) {
            ...
            esp_err_t anim_ret = touch_led_start_animation(saved_config.animation, saved_config.animation_speed,
                                                          saved_config.animation_primary, saved_config.animation_secondary);
            if (anim_ret == ESP_OK) {
                ESP_LOGI(TAG, "Restored animation: %d at speed %d", saved_config.animation, saved_config.animation_speed);
            }
            ...
            if (color_ret == ESP_OK) {
                ...
                ESP_LOGI(TAG, "Restored static color: RGB(%d,%d,%d)", 
                        saved_config.static_color.red, saved_config.static_color.green, saved_config.static_color.blue);
            }
        }
...
esp_err_t touch_led_reset_config(void)
{
    ...
    esp_err_t ret = config_manager_delete(TOUCH_LED_CONFIG_NAMESPACE, TOUCH_LED_CONFIG_KEY);
