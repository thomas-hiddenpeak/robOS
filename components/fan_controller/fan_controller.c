...
static esp_err_t fan_controller_update_pwm(uint8_t fan_id, uint8_t speed_percent);
static esp_err_t fan_controller_apply_curve(uint8_t fan_id, float temperature);
static uint8_t fan_controller_interpolate_speed(const fan_curve_point_t *curve, uint8_t num_points, float temperature);
...
fan_controller_config_t fan_controller_get_default_config(void)
{
    fan_controller_config_t config = {
        ...
        .update_interval_ms = FAN_CONTROLLER_DEFAULT_UPDATE_INTERVAL
    };
    return config;
}
...
esp_err_t fan_controller_init(const fan_controller_config_t *config)
{
    ...
    // Initialize fans with default configuration (will be overridden by loaded config if available)
    ...
    BaseType_t ret = xTaskCreate(
        fan_controller_task,
        "fan_controller",
        FAN_CONTROLLER_TASK_STACK_SIZE / sizeof(StackType_t),
        NULL,
        FAN_CONTROLLER_TASK_PRIORITY,
        &s_fan_ctx.task_handle
    );
    ...
    ESP_LOGI(TAG, "Fan controller initialized successfully with %d fans", s_fan_ctx.num_fans);
...
esp_err_t fan_controller_configure_gpio(uint8_t fan_id, int pwm_pin, int pwm_channel)
{
    ...
    hal_pwm_config_t pwm_config = {
        ...
        .duty_cycle = (fan->config.default_speed * ((1 << FAN_CONTROLLER_PWM_RESOLUTION) - 1)) / 100,
        .invert = fan->config.invert_pwm
    };
    
    esp_err_t ret = hal_pwm_configure(&pwm_config);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to configure PWM on new pin %d: %s", pwm_pin, esp_err_to_name(ret));
        fan->status.enabled = false;
        fan->status.fault = true;
    } else {
        ESP_LOGI(TAG, "Fan %d reconfigured: GPIO%d, Channel %d", fan_id, pwm_pin, fan->config.pwm_channel);
...
esp_err_t fan_controller_set_curve(uint8_t fan_id, const fan_curve_point_t *curve_points, uint8_t num_points)
{
    ...
    fan->curve_points = (fan_curve_point_t *)malloc(sizeof(fan_curve_point_t) * num_points);
    if (fan->curve_points == NULL) {
    ...
    memcpy(fan->curve_points, curve_points, sizeof(fan_curve_point_t) * num_points);
    ...
    for (uint8_t i = 0; i < num_points - 1; i++) {
        for (uint8_t j = 0; j < num_points - i - 1; j++) {
            if (fan->curve_points[j].temperature > fan->curve_points[j + 1].temperature) {
    ...
    if (save_ret == ESP_OK) {
        ...
        ESP_LOGW(TAG, "Failed to save fan %d curve configuration: %s", fan_id, esp_err_to_name(save_ret));
    }
...
esp_err_t fan_controller_register_commands(void)
{
    ...
    console_cmd_t fan_commands[] = {
        {
            .command = "fan",
            .help = "fan <command> [args...] - PWM fan controller (type 'fan help' for details)",
...
static float get_fan_temperature_for_mode(uint8_t fan_id) {
    // For curve mode, use test temperature if available
    int test_temp = 25;
    if (console_get_test_temperature(&test_temp) == ESP_OK) {
        return (float)test_temp;
    }
    // Otherwise, use current status temperature
    if (fan_id < s_fan_ctx.num_fans) {
...
static esp_err_t fan_controller_update_pwm(uint8_t fan_id, uint8_t speed_percent)
{
    ...
    uint8_t actual_speed = fan->config.invert_pwm ? (100 - speed_percent) : speed_percent;
    ...
    if (ret == ESP_OK) {
        ...
        ESP_LOGW(TAG, "LEDC not initialized for fan %d, attempting to reinitialize PWM", fan_id);
        ...
        if (init_ret == ESP_OK) {
            ...
            ESP_LOGE(TAG, "Failed to reinitialize PWM for fan %d: %s", fan_id, esp_err_to_name(init_ret));
            fan->status.fault = true;
            ret = init_ret;
        }
    } else {
        ESP_LOGE(TAG, "Failed to set PWM duty for fan %d: %s", fan_id, esp_err_to_name(ret));
        fan->status.fault = true;
    }
...
static esp_err_t fan_controller_apply_curve(uint8_t fan_id, float temperature)
{
    ...
    uint8_t speed = fan_controller_interpolate_speed(fan->curve_points, 
...
static uint8_t fan_controller_interpolate_speed(const fan_curve_point_t *curve, uint8_t num_points, float temperature)
{
    ...
    for (uint8_t i = 0; i < num_points - 1; i++) {
        if (temperature >= curve[i].temperature && temperature <= curve[i + 1].temperature) {
            ...
            return curve[i].speed_percent + (uint8_t)((temp_offset / temp_range) * speed_range);
        }
    }
        ...
        for (uint8_t i = 0; i < s_fan_ctx.num_fans; i++) {
            ...
            if (ret == ESP_OK) {
                ...
                switch (status.mode) {
                    ...
                    case FAN_MODE_AUTO_TEMP: mode_str = "Auto-Temp"; break;
                    case FAN_MODE_AUTO_CURVE: mode_str = "Auto-Curve"; break;
...
static esp_err_t cmd_fan_gpio(int argc, char **argv)
{
    if (argc < 3) {
        ...
        printf("Examples:\n");
        printf("  fan gpio 0 41     # Configure fan 0 to GPIO41 (keep current channel)\n");
...
static esp_err_t cmd_fan_config(int argc, char **argv)
{
    if (argc < 2) {
        ...
        printf("Commands:\n");
        printf("  save [fan_id]  - Save complete fan configuration(s) to storage\n");
        printf("                   (includes hardware config, mode, speed, enable state)\n");
        printf("  load [fan_id]  - Load complete fan configuration(s) from storage\n");
        printf("  show [fan_id]  - Show current fan configuration(s)\n");
        printf("  curve <fan_id> <temp1:speed1> [temp2:speed2] ... - Set temperature curve\n");
        printf("Examples:\n");
        printf("  fan config save     # Save all fan configurations with runtime state\n");
        ...
        printf("  fan config curve 0 30:20 50:30 70:40 80:100  # Set curve: 30°C->20%%, 50°C->30%%, 70°C->40%%, 80°C->100%%\n");
    ...
    if (strcmp(action, "save") == 0) {
        ...
        if (fan_id >= 0) {
            ...
            if (ret == ESP_OK) {
                ...
                fan_status_t status;
                if (fan_controller_get_status(fan_id, &status) == ESP_OK) {
                    const char *mode_str = (status.mode == FAN_MODE_MANUAL) ? "Manual" :
                                         (status.mode == FAN_MODE_AUTO_TEMP) ? "Auto-Temp" :
                                         (status.mode == FAN_MODE_AUTO_CURVE) ? "Auto-Curve" : "Off";
                    printf("  Saved: Mode=%s, Speed=%d%%, Enabled=%s\n", 
                           mode_str, status.speed_percent, status.enabled ? "Yes" : "No");
                }
            } else {
        ...
        if (fan_id >= 0) {
            ...
            const char *mode_str = (fan->status.mode == FAN_MODE_MANUAL) ? "Manual" :
                                 (fan->status.mode == FAN_MODE_AUTO_TEMP) ? "Auto-Temp" :
                                 (fan->status.mode == FAN_MODE_AUTO_CURVE) ? "Auto-Curve" : "Off";
            printf("    Mode: %s\n", mode_str);
            ...
            if (fan->curve_enabled && fan->num_curve_points > 0) {
                ...
                for (uint8_t j = 0; j < fan->num_curve_points; j++) {
                    printf("    %.1f°C -> %d%%\n", fan->curve_points[j].temperature, fan->curve_points[j].speed_percent);
                }
            } else {
                printf("    Enabled: No\n");
            }
            ...
            for (uint8_t i = 0; i < s_fan_ctx.num_fans; i++) {
                ...
                printf("Fan %d: GPIO%d, Ch%d, %s, Speed%d%%, %s\n", 
                       i, fan->config.pwm_pin, fan->config.pwm_channel,
                       mode_str, fan->status.speed_percent,
        ...
        if (argc < 4) {
            ...
            printf("  Sets temperature curve: 30°C->20%%, 50°C->30%%, 70°C->40%%, 80°C->100%%\n");
            return ESP_ERR_INVALID_ARG;
        }
        ...
        fan_curve_point_t *curve_points = (fan_curve_point_t *)malloc(sizeof(fan_curve_point_t) * num_points);
        ...
        if (ret == ESP_OK) {
            ...
            for (uint8_t i = 0; i < num_points; i++) {
                printf("  %.1f°C -> %d%%\n", curve_points[i].temperature, curve_points[i].speed_percent);
            }
...
static esp_err_t cmd_fan_help(void)
{
    ...
    printf("                      Points are automatically sorted by temperature\n");
    printf("\n");
    printf("  help\n");
    ...
    printf("\n");
    printf("  # Use with test command for curve debugging\n");
    printf("  test temp 45    # Set test temperature to 45°C\n");
    printf("  fan mode 0 curve  # Fan will use test temperature\n");
    printf("\n");
    printf("NOTES\n");
    printf("\n");
    ...
    printf("  • Use 'test temp <value>' to set debug temperature for curve mode\n");
...
static esp_err_t save_fan_config(uint8_t fan_id)
{
    ...
    esp_err_t ret = config_manager_set(FAN_CONFIG_NAMESPACE, key, CONFIG_TYPE_BLOB,
    ...
    if (ret == ESP_OK) {
        ...
        ESP_LOGE(TAG, "Failed to save fan %d hardware config: %s", fan_id, esp_err_to_name(ret));
    }
...
static esp_err_t save_fan_full_config(uint8_t fan_id)
{
    ...
    if (s_fan_ctx.fans[fan_id].curve_points && s_fan_ctx.fans[fan_id].num_curve_points > 0) {
    ...
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "Fan %d full configuration saved (Mode:%d, Speed:%d%%, Enabled:%s)", 
                fan_id, full_config.current_mode, full_config.current_speed,
                full_config.enabled ? "Yes" : "No");
        config_manager_commit();
    } else {
        ESP_LOGE(TAG, "Failed to save fan %d full config: %s", fan_id, esp_err_to_name(ret));
    }
...
static esp_err_t load_fan_config(uint8_t fan_id)
{
    ...
    esp_err_t ret = config_manager_get(FAN_CONFIG_NAMESPACE, key, CONFIG_TYPE_BLOB,
    ...
    if (ret == ESP_OK) {
        ...
        if (s_fan_ctx.fans[fan_id].config.pwm_pin >= 0) {
            hal_pwm_config_t pwm_config = {
                ...
                .frequency = FAN_CONTROLLER_PWM_FREQUENCY,
                .resolution = FAN_CONTROLLER_PWM_RESOLUTION,
                .duty_cycle = (s_fan_ctx.fans[fan_id].config.default_speed * ((1 << FAN_CONTROLLER_PWM_RESOLUTION) - 1)) / 100,
                .invert = s_fan_ctx.fans[fan_id].config.invert_pwm
            };
            
            esp_err_t pwm_ret = hal_pwm_configure(&pwm_config);
            if (pwm_ret != ESP_OK) {
                ESP_LOGE(TAG, "Failed to reconfigure PWM for fan %d: %s", fan_id, esp_err_to_name(pwm_ret));
        ...
        ESP_LOGI(TAG, "No saved hardware configuration found for fan %d, initializing with defaults", fan_id);
        ...
        if (s_fan_ctx.fans[fan_id].config.pwm_pin >= 0) {
            ...
            esp_err_t pwm_ret = hal_pwm_configure(&pwm_config);
            if (pwm_ret != ESP_OK) {
                ESP_LOGE(TAG, "Failed to initialize default PWM for fan %d: %s", fan_id, esp_err_to_name(pwm_ret));
                s_fan_ctx.fans[fan_id].status.enabled = false;
                s_fan_ctx.fans[fan_id].status.fault = true;
            } else {
                ESP_LOGI(TAG, "Fan %d initialized with default PWM: GPIO%d, Channel %d", 
                        fan_id, s_fan_ctx.fans[fan_id].config.pwm_pin, s_fan_ctx.fans[fan_id].config.pwm_channel);
                s_fan_ctx.fans[fan_id].status.enabled = true;
                s_fan_ctx.fans[fan_id].status.fault = false;
        ...
        ESP_LOGE(TAG, "Failed to load fan %d hardware config: %s", fan_id, esp_err_to_name(ret));
    }
...
static esp_err_t load_fan_full_config(uint8_t fan_id)
{
    ...
    esp_err_t ret = config_manager_get(FAN_CONFIG_NAMESPACE, key, CONFIG_TYPE_BLOB,
                                      &full_config, &config_size);

    if (ret == ESP_OK) {
        // Check version compatibility
        if (full_config.version < 1 || full_config.version > 2) {
            ESP_LOGW(TAG, "Fan %d config version mismatch, using hardware config only", fan_id);
        ...
        if (full_config.version >= 2) {
            ...
            if (full_config.num_curve_points > 0 && full_config.curve_enabled) {
                s_fan_ctx.fans[fan_id].curve_points = malloc(sizeof(fan_curve_point_t) * full_config.num_curve_points);
                if (s_fan_ctx.fans[fan_id].curve_points) {
                    memcpy(s_fan_ctx.fans[fan_id].curve_points, full_config.curve_points,
                           sizeof(fan_curve_point_t) * full_config.num_curve_points);
                    ESP_LOGI(TAG, "Fan %d temperature curve loaded with %d points", fan_id, full_config.num_curve_points);
                } else {
                    ESP_LOGE(TAG, "Failed to allocate memory for fan %d curve points", fan_id);
        ...
        ESP_LOGI(TAG, "Fan %d full configuration loaded: GPIO%d, Mode:%d, Speed:%d%%, Enabled:%s, Curve:%s (%d points)", 
        ...
        if (s_fan_ctx.fans[fan_id].config.pwm_pin >= 0) {
            hal_pwm_config_t pwm_config = {
                ...
                .frequency = FAN_CONTROLLER_PWM_FREQUENCY,
                .resolution = FAN_CONTROLLER_PWM_RESOLUTION,
                .duty_cycle = (full_config.current_speed * ((1 << FAN_CONTROLLER_PWM_RESOLUTION) - 1)) / 100,
                .invert = s_fan_ctx.fans[fan_id].config.invert_pwm
            };
        ...
        ESP_LOGI(TAG, "No saved full configuration found for fan %d, trying hardware config", fan_id);
        return load_fan_config(fan_id);  // Fallback to hardware config which will init defaults if needed
    } else {
        ESP_LOGE(TAG, "Failed to load fan %d full config: %s", fan_id, esp_err_to_name(ret));
    }
