...
#define FAN_CONTROLLER_PWM_FREQUENCY                                           \
  25000 ///< PWM frequency for fan control (25kHz)
#define FAN_CONTROLLER_PWM_RESOLUTION                                          \
  LEDC_TIMER_10_BIT ///< PWM resolution (10-bit)

// Temperature control constants
#define FAN_CONTROLLER_DEFAULT_TEMP_HYSTERESIS                                 \
  3.0f ///< Default temperature hysteresis (°C)
#define FAN_CONTROLLER_MIN_SPEED_CHANGE_INTERVAL                               \
  2000 ///< Minimum interval between speed changes (ms)
 ...
 * @param status_array Array to store fan status (must be sized for configured
 * fans)
...
/**
 * @brief Configure temperature hysteresis and rate limiting for a fan
 * @param fan_id Fan ID (0-3)
 * @param temperature_hysteresis Temperature dead zone in °C (0.0 to disable)
 * @param min_speed_change_interval Minimum interval between speed changes in ms
 * @return ESP_OK on success, error code on failure
 */
esp_err_t
